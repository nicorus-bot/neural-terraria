...
const App = () => {
    ...
    const [selectedSlot, setSelectedSlot] = useState(2); // 最初はゼニス
    ...
    const startBGM = useCallback(() => {
        ...
        const melody = [{f:261,d:0.4},{f:329,d:0.4},{f:392,d:0.4},{f:523,d:0.8}];
        let nextTime = ctx.currentTime;
        const loop = () => {
            melody.forEach(n => {
                const osc = ctx.createOscillator(); const gain = ctx.createGain();
                osc.type = 'triangle'; osc.frequency.setValueAtTime(n.f, nextTime);
                gain.gain.setValueAtTime(0.02, nextTime); gain.gain.exponentialRampToValueAtTime(0.0001, nextTime + n.d);
                osc.connect(gain); gain.connect(ctx.destination);
                osc.start(nextTime); osc.stop(nextTime + n.d);
                nextTime += n.d;
            });
    ...
    return (
        <div style={{ width: '100vw', height: '100vh', overflow: 'hidden', position: 'relative', touchAction: 'none', background: '#000', userSelect: 'none' }}>
            ...
            <div style={{ position: 'fixed', top: '20px', right: '20px', textAlign: 'right', zIndex: 100 }}>
                <div style={{ color: '#fff', fontSize: '1.2em', fontWeight: 'bold', fontFamily: 'Orbitron' }}>NEURAL TERRARIA v19</div>
            ...
            {isMobile && (
                <div style={{ position: 'fixed', bottom: '30px', left: '30px', right: '30px', display: 'flex', justifyContent: 'space-between', zIndex: 100, pointerEvents: 'none' }}>
                    <div style={{ display: 'flex', gap: '15px', pointerEvents: 'auto' }}>
                        <button onTouchStart={() => keysRef.current.left = true} onTouchEnd={() => keysRef.current.left = false} style={btnStyle}>←</button>
                        <button onTouchStart={() => keysRef.current.right = true} onTouchEnd={() => keysRef.current.right = false} style={btnStyle}>→</button>
                    </div>
                    <button onTouchStart={() => keysRef.current.jump = true} onTouchEnd={() => keysRef.current.jump = false} style={{ ...btnStyle, borderRadius: '50%', width: '90px', height: '90px' }}>UP</button>
                </div>
            )}
        </div>
    );
...
const btnStyle = { width: '70px', height: '70px', background: 'rgba(255,255,255,0.2)', border: '2px solid #fff', borderRadius: '15px', color: '#fff', fontSize: '2em', display: 'flex', alignItems: 'center', justifyContent: 'center', pointerEvents: 'auto', WebkitTapHighlightColor: 'transparent' };
